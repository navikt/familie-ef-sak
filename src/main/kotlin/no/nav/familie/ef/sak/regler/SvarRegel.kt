...
data class SluttSvarRegel private constructor(@JsonIgnore
    ...
    companion object {
        ...
        val OPPFYLT_MED_PÅKREVD_BEGRUNNELSE = SluttSvarRegel(resultat = Resultat.OPPFYLT,
                                                             begrunnelseType = BegrunnelseType.PÅKREVD)
        val OPPFYLT_MED_VALGFRI_BEGRUNNELSE = SluttSvarRegel(resultat = Resultat.OPPFYLT,
                                                             begrunnelseType = BegrunnelseType.VALGFRI)

        val IKKE_OPPFYLT = SluttSvarRegel(resultat = Resultat.IKKE_OPPFYLT)
        val IKKE_OPPFYLT_MED_PÅKREVD_BEGRUNNELSE = SluttSvarRegel(resultat = Resultat.IKKE_OPPFYLT,
                                                                  begrunnelseType = BegrunnelseType.PÅKREVD)
        val IKKE_OPPFYLT_MED_VALGFRI_BEGRUNNELSE = SluttSvarRegel(resultat = Resultat.IKKE_OPPFYLT,
                                                                  begrunnelseType = BegrunnelseType.VALGFRI)
    }
