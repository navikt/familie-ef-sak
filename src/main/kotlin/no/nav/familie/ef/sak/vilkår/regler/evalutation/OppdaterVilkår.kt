...
object OppdaterVilkår {
    ...
    fun utledBehandlingKategori(vilkårsvurderinger: List<Vilkårsvurdering>): BehandlingKategori {
        val medlemFolketrygd =
            vilkårsvurderinger.utledVurderinger(VilkårType.FORUTGÅENDE_MEDLEMSKAP, RegelId.SØKER_MEDLEM_I_FOLKETRYGDEN).harSvar(
                SvarId.JA,
            )

        val unntakEøsAnnenForelder =
            vilkårsvurderinger.utledVurderinger(VilkårType.FORUTGÅENDE_MEDLEMSKAP, RegelId.MEDLEMSKAP_UNNTAK).harSvar(
        ...
        val borOgOppholderSegINorge =
            vilkårsvurderinger.utledVurderinger(VilkårType.LOVLIG_OPPHOLD, RegelId.BOR_OG_OPPHOLDER_SEG_I_NORGE).harSvar(
                SvarId.JA,
            )
