...
sealed class FeatureToggle(
    ...
    companion object {
        private val togglesById = values().associateBy { it.toggleId }

        fun fraToggleId(toggleId: String): FeatureToggle = togglesById[toggleId] ?: error("Finner ikke toggle for $toggleId")

        fun values(): List<FeatureToggle> =
            listOf(
                FrontendKonverterDelmalblokkTilHtmlFelt,
                FrontendVisMarkereGodkjenneOppgaveModal,
                KontrollerNæringsinntekt,
                GBeregning,
                GBeregningScheduler,
                SatsendringBrukIkkeVedtattMaxsats,
                FrontendVisIkkePubliserteBrevmaler,
                FrontendAutomatiskUtfylleVilkar,
                AutomatiskeBrevInnhentingAktivitetsplikt,
                FrontendVisTildelOppgaveBehandling,
                MigreringBarnetilsyn,
                OpprettBehandlingFerdigstiltJournalpost,
                BehandlingKorrigering,
                FrontendSatsendring,
                TillatMigrering7ÅrTilbake,
                UtviklerMedVeilederrolle,
                HenleggBehandlingUtenOppgave,
                VelgÅrsakVedKlageOpprettelse,
                VisSamværskalkulator,
                VisAutomatiskInntektsendring,
            )
    }
}

enum class FeatureToggleBeskrivelse(
