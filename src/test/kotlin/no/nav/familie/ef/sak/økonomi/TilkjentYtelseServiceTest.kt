...
class TilkjentYtelseServiceTest {
    ...
    inner class FinnTilkjentYtelserTilKonsistensavstemming {
        ...
        internal fun `deler opp kall mot service i bolker`() {
            ...
            verify {
                behandlingService.hentBehandlinger(
                    ytelser
                        .subList(0, PdlClient.MAKS_ANTALL_IDENTER)
                        .map { it.behandlingId }
                        .toSet(),
                )
            }
            verify {
                behandlingService.hentBehandlinger(
                    ytelser
                        .subList(PdlClient.MAKS_ANTALL_IDENTER, 110)
                        .map { it.behandlingId }
                        .toSet(),
                )
            }
        ...
        internal fun `filtrer bort andeler som har 0-beløp`() {
            ...
            every { fagsakService.fagsakerMedOppdatertePersonIdenter(listOf(behandling.fagsakId)) } returns listOf(fagsak)
        ...
        internal fun `skal kaste feil hvis den ikke finner eksterneIder til behandling`() {
            ...
            val tilkjentYtelse =
                DataGenerator
                    .tilfeldigTilkjentYtelse(behandling)
