...
class VedtakServiceTest {
    ...
    private val periodeUtenInntekt = inntektsperiode(LocalDate.of(2021, 1, 1), LocalDate.of(2021, 12, 31), BigDecimal.ZERO)
    private val periodeMedInntekt = inntektsperiode(LocalDate.of(2022, 1, 1), LocalDate.of(2024, 12, 31), BigDecimal.valueOf(400000))
    private val inntektWrapper = InntektWrapper(listOf(periodeUtenInntekt, periodeMedInntekt))

    @BeforeEach
    fun setUp() {
        every { vedtakService.hentVedtak(behandlingId) } returns vedtak(behandlingId = behandlingId, inntekter = inntektWrapper)
    }
    ...
    fun `finn inntekt for tidspunkt`() {
        ...
        val inntektPaaTidspunktUtenInntekt =
                vedtakService.hentForventetInntektForVedtakOgDato(behandlingId, LocalDate.of(2021, 1, 1))
